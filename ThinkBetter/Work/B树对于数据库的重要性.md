...
![[Pasted image 20251128093239.png|500]]

插入过程中节点分裂。完整节点在中间点分裂，中间键（30）被提升为父节点。
**当分裂传播到根节点时：**

- 根节点被分成两个节点
- 使用一个密钥（从旧根节点提升的密钥）创建一个新根节点。
- 树高增加 1

这是B型树增高的唯一方式。B**型树是从叶子向上生长，而不是从根部向下生长。**

## B树节点合并

当您从节点中删除一个键，导致该节点容量过低（低于 50%）时，它将与一个同级节点合并。

**合并算法：**

1. 将所有键从右侧兄弟节点复制到左侧兄弟节点
2. 将分隔符键从父节点降级到合并节点
3. 删除右兄弟节点
4. 如果父级变得太空，则递归地合并它。

![[Pasted image 20251128093448.png|400]]

删除过程中的节点合并。当右侧节点为空过多时，它会与左侧节点合并，并从父节点提取分隔符键。

**当合并操作传播到根目录时：**

- 合并后，如果根节点只有一个子节点，则该子节点成为新的根节点。
- 树高减少了1

分支和合并操作可以保持树的平衡。所有叶子节点都保持在相同的深度，从而确保查询性能的一致性。
